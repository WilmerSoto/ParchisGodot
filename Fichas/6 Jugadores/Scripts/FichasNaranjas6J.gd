...
class_name FichasNaranjas6J
var dadosInicio
var colorTurno = Color8(255,127,3)
...
func _ready():
	...
	listaMovimientos = get_parent().get_parent().get_node("Movimientos/MovimientosNaranja").get_curve()
...
func play_turn(dado1, dado2, suma):
	...
	listaSalidas = get_parent().get_parent().get_node("Movimientos/SalidaNaranja")
	...
	if turnoEnCurso == true and organizandoFichas == false:
		...
		if contadorPares == 3:
			...
			for x in Variablesjuego.get_numFichas():
				get_child(x).get_node("Naranja").texture = load("res://Fichas/Imagenes/NaranjaMovimiento.png")
			...
			for x in Variablesjuego.get_numFichas():
				get_child(x).get_node("Naranja").texture = load("res://Fichas/Imagenes/Naranja.png")
...
func moverFicha(cantidad, fichaAMover, indiceFicha, listaMovimientos):
	...
	if posActual+cantidad in [3,6,9,12,15]:
		...
		match posActual+cantidad:
			3:
				posSalidas = get_parent().get_parent().get_node("Movimientos/SalidaMorado").get_children()
			6:
		...
		1,2,10,11:
			...
			if not interseccion.empty():
				...
				if not interseccion.empty():
					contadorIntersecciones += 1
		3:
			var salidasAzul = get_parent().get_parent().get_node("Movimientos/SalidaMorado").get_children()
			for i in Variablesjuego.get_numFichas():
		...
		6:
			...
			for i in Variablesjuego.get_numFichas():
				...
				if not interseccion.empty():
					contadorIntersecciones += 1
		9:
			var salidasRojo = get_parent().get_parent().get_node("Movimientos/SalidaAzul").get_children()
			for i in Variablesjuego.get_numFichas():
				interseccion = space.intersect_point(salidasRojo[i].get_position(), 1, [], 2147483647, false, true)
				if not interseccion.empty():
					contadorIntersecciones += 1
		12:
			var salidasAzul = get_parent().get_parent().get_node("Movimientos/SalidaRojo").get_children()
			for i in Variablesjuego.get_numFichas():
				interseccion = space.intersect_point(salidasAzul[i].get_position(), 1, [], 2147483647, false, true)
				if not interseccion.empty():
					contadorIntersecciones += 1
...
func moverACarcel(indice):
	...
	var listaSalida = get_parent().get_parent().get_node("PosIniciales/Naranja").get_children()
