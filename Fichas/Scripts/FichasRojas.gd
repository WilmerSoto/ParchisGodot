...
class_name FichasRojas
var dadosInicio
var colorTurno = Color.red
...
func _ready():
	...
	listaMovimientos = get_parent().get_parent().get_node("Movimientos/MovimientosRojo").get_curve()
...
func play_turn(dado1, dado2, suma):
	...
	listaSalidas = get_parent().get_parent().get_node("Movimientos/SalidaRojo")
	...
	if turnoEnCurso == true and organizandoFichas == false:
		...
		if contadorPares == 3:
			...
			for x in Variablesjuego.get_numFichas():
				get_child(x).get_node("Rojo").texture = load("res://Fichas/Imagenes/RojoMovimiento.png")
			...
			for x in Variablesjuego.get_numFichas():
				get_child(x).get_node("Rojo").texture = load("res://Fichas/Imagenes/Rojo.png")
...
func moverACarcel(indice):
	...
	var listaSalida = get_parent().get_parent().get_node("PosIniciales/Rojo").get_children()
