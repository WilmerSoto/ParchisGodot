...
class_name FichasAmarillas
var dadosInicio
var colorTurno = Color.yellow
...
func _ready():
	...
	listaMovimientos = get_parent().get_parent().get_node("Movimientos/MovimientosAmarillo").get_curve()
...
func play_turn(dado1, dado2, suma):
	...
	listaSalidas = get_parent().get_parent().get_node("Movimientos/SalidaAmarillo")
	...
	if turnoEnCurso == true and organizandoFichas == false:
		...
		if contadorPares == 3:
			...
			for x in Variablesjuego.get_numFichas():
				get_child(x).get_node("Amarillo").texture = load("res://Fichas/Imagenes/AmarillaMovimiento.png")
			...
			for x in Variablesjuego.get_numFichas():
				get_child(x).get_node("Amarillo").texture = load("res://Fichas/Imagenes/Amarilla.png")
...
func moverFicha(cantidad, fichaAMover, indiceFicha, listaMovimientos):
	...
	if cantidad == 0:
		...
		var lista = listaSalidas.get_children()
		for i in Variablesjuego.get_numFichas():
			if salidaPos[i] == false:
				fichaAMover.set_position(lista[i].get_position())
				break
...
func moverACarcel(indice):
	...
	var listaSalida = get_parent().get_parent().get_node("PosIniciales/Amarillo").get_children()
