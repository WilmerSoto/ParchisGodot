...
class_name FichasAzules
var dadosInicio
var colorTurno = Color8(0, 172, 255)
...
func _ready():
	...
	listaMovimientos = get_parent().get_parent().get_node("Movimientos/MovimientosAzul").get_curve()
...
func play_turn(dado1, dado2, suma):
	...
	listaSalidas = get_parent().get_parent().get_node("Movimientos/SalidaAzul")
	...
	if turnoEnCurso == true and organizandoFichas == false:
		...
		if contadorPares == 3:
			...
			for x in Variablesjuego.get_numFichas():
				get_child(x).get_node("Azul").texture = load("res://Fichas/Imagenes/AzulMovimiento.png")
			...
			for x in Variablesjuego.get_numFichas():
				get_child(x).get_node("Azul").texture = load("res://Fichas/Imagenes/Azul.png")
...
func moverFicha(cantidad, fichaAMover, indiceFicha, listaMovimientos):
	...
	if posActual+cantidad in [3,6,9]:
		var posSalidas
		var salidasPos
		var interseccion
			...
			if ficha == curve2DFicha[indiceFicha]:
				if indiceBusqueda != indiceFicha:
					match curve2DFicha[indiceFicha]:
						...
						_:
							fichaAMover.set_position(fichaAMover.get_position() - Vector2(0,15))
							get_child(indiceBusqueda).set_position(get_child(indiceBusqueda).get_position() + Vector2(0,15))
			indiceBusqueda += 1
	...
	if curve2DFicha[indiceFicha] == 10:
		posFichas[indiceFicha] = 3
	if curve2DFicha[indiceFicha] == 11:
		posFichas[indiceFicha] = 4
	if curve2DFicha[indiceFicha] == 12:
		fichasEnMeta.append(fichaAMover)
		fichaAMover.set_visible(false)
		...
		3:
			...
			for i in Variablesjuego.get_numFichas():
				...
				if not interseccion.empty():
					contadorIntersecciones += 1
		6:
			var salidasVerde = get_parent().get_parent().get_node("Movimientos/SalidaVerde").get_children()
			for i in Variablesjuego.get_numFichas():
				interseccion = space.intersect_point(salidasVerde[i].get_position(), 1, [], 2147483647, false, true)
				if not interseccion.empty():
					contadorIntersecciones += 1
		9:
			var salidasAmarillo = get_parent().get_parent().get_node("Movimientos/SalidaAmarillo").get_children()
			for i in Variablesjuego.get_numFichas():
				interseccion = space.intersect_point(salidasAmarillo[i].get_position(), 1, [], 2147483647, false, true)
				if not interseccion.empty():
					contadorIntersecciones += 1
		_:
			interseccion = space.intersect_point(mov + Vector2(0,15), 3, [], 2147483647, false, true)
			if not interseccion.empty():
				contadorIntersecciones += 1
				interseccion = space.intersect_point(mov - Vector2(0,15), 3, [], 2147483647, false, true)
				if not interseccion.empty():
...
func moverACarcel(indice):
	...
	var listaSalida = get_parent().get_parent().get_node("PosIniciales/Azul").get_children()
