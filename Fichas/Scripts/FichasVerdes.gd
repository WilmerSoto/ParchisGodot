...
class_name FichasVerdes
var dadosInicio
var colorTurno = Color8(49,173,31)
...
func _ready():
	...
	listaMovimientos = get_parent().get_parent().get_node("Movimientos/MovimientosVerde").get_curve()
...
func play_turn(dado1, dado2, suma):
	...
	listaSalidas = get_parent().get_parent().get_node("Movimientos/SalidaVerde")
	...
	if turnoEnCurso == true and organizandoFichas == false:
		...
		if contadorPares == 3:
			...
			for x in Variablesjuego.get_numFichas():
				get_child(x).get_node("Verde").texture = load("res://Fichas/Imagenes/VerdeMovimiento.png")
			...
			for x in Variablesjuego.get_numFichas():
				get_child(x).get_node("Verde").texture = load("res://Fichas/Imagenes/Verde.png")
...
func moverACarcel(indice):
	...
	var listaSalida = get_parent().get_parent().get_node("PosIniciales/Verde").get_children()
